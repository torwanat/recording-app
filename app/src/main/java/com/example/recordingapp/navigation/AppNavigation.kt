...
import com.example.recordingapp.gallery.GalleryScreen
...
fun AppNavigation(navController: NavHostController, modifier: Modifier) {
    NavHost(navController = navController, startDestination = Routes.AUDIO, modifier = modifier) {
        ...
        composable(Routes.IMAGE) { ImageScreen(onNavigateToAudio = { navController.popBackStack() }, onNavigateToGallery = { navController.navigate(Routes.GALLERY) }) }
        composable(Routes.GALLERY) { GalleryScreen(onNavigateToImage = { navController.popBackStack() }) }
    }
}